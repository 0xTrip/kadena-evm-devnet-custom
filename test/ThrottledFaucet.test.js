...
const { deployContractOnChains, switchChain } = chainweb;
const {
  loadFixture,
...
describe("ThrottledFaucet", function () {
  ...
  async function deployFaucetFixture() {
    ...
    await switchChain(0);
    const [deployer, faucetWallet, recipient, otherAccount1] =
    ...
    const deployments = await Promise.all(
      deployed.deployments.map(async (deployment) => {
        ...
        await grantTx1.wait();
        const grantTx2 = await faucet.grantRole(
          FAUCET_ROLE,
          faucetWallet.address
        );
        await grantTx2.wait();

        expect(await faucet.hasRole(ADMIN_ROLE, deployer.address)).to.be.true;
        expect(await faucet.hasRole(FAUCET_ROLE, faucetWallet.address)).to.be
          .true;
        ...
        console.log(
          `Contract balance after funding: ${ethers.formatEther(
            contractBalance
          )} KDA`
        );
      ...
      it("Should dispense tokens to valid recipient", async function () {
        ...
        for (const deployment of deployments) {
          ...
          await faucet
            ...
            .dispenseNativeToken(recipient.address);
    ...
    describe("Error Test Cases", function () {
      it("Should revert when called by non-admin", async function () {
        ...
        for (const deployment of deployments) {
          ...
          await expect(faucet.connect(otherAccount1).setCooldownPeriod(7200))
            .to.be.revertedWithCustomError(
              faucet,
              "AccessControlUnauthorizedAccount"
            )
            .withArgs(otherAccount1.address, ADMIN_ROLE);
        }
      });
    });
  });
    ...
    describe("Success Test Cases", function () {
      it("Should update native token amount when called by admin", async function () {
        ...
        for (const deployment of deployments) {
          ...
          const tx = await faucet
            .connect(deployer)
            .setNativeTokenAmount(newAmount);
          await tx.wait();
    ...
    describe("Error Test Cases", function () {
      it("Should revert when called by non-admin", async function () {
        ...
        for (const deployment of deployments) {
          ...
          await expect(faucet.connect(otherAccount1).setNativeTokenAmount(100))
            .to.be.revertedWithCustomError(
              faucet,
              "AccessControlUnauthorizedAccount"
            )
